...
function addImageWithFrame(slide, path, x, y, w, h, caption = "") {
  const assetName = String(path).split("/").at(-1) || "asset";
  slide.addShape("rect", {
    x,
    y,
    w,
  ...
  addTag(
    slide,
    "Recovered Figure Slot",
    x + 0.22,
    y + 0.18,
    Math.min(1.72, w - 0.44),
    COLORS.tealSoft,
    COLORS.teal,
  );
  slide.addText("原 deck 资产已保留在工作目录中", {
    x: x + 0.2,
    y: y + h * 0.38,
    w: w - 0.4,
    h: 0.18,
  ...
  slide.addText(
    "当前运行时的 PptxGenJS 图片嵌入会生成 PowerPoint 无法打开的文件，\n因此这里保留图位与说明，原始图片文件一并随源代码交付。",
    {
      x: x + 0.24,
      y: y + h * 0.62,
      w: w - 0.48,
