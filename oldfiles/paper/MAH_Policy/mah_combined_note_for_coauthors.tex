...
\ifXeTeX\else\ifLuaTeX\else\PackageError{mah-combined-note}{Please compile this file with XeLaTeX or LuaLaTeX to use explicit CJK fonts}{}\fi\fi
...
\begin{longtable}{@{}P{0.19\textwidth}P{0.21\textwidth}P{0.52\textwidth}@{}}
...
$\Pi_A(z),\Pi_B(z),$\newline$\Pi_C(z)$ & Optimized flow payoff objects & Current-period payoffs that enter the Bellman equation. \\[0.4em]

$V_A(z),V_B(z),$\newline$V_C(z)$ & Value functions & Continuation values under each organizational type. \\
...
\[
\begin{aligned}
V_A(z)=\max\bigl\{\,&\Pi_A(z)+\beta E[V_A(z')\mid z,A],\\
...
\[
\begin{aligned}
V_B(z)=\max\bigl\{\,&\Pi_B(z)+\beta E[V_B(z')\mid z,B],\\
...
\[
\begin{aligned}
V_C(z)=\max\bigl\{\,&\Pi_C(z)+\beta E[V_C(z')\mid z,C],\\
