...
viable drug-project advancement, commercialization organization, and
...
There is a continuum \(\mathcal I\) of drug developers.  Developer \(i\) has research capability \(a_i>0\), measured in \(\mathsf P/\mathsf X\), and
internal manufacturing capability \(k_i>0\), measured in \(\mathsf K\).  Its
characteristics follow the exogenous distribution \(H(a,k)\). Its complete
permanent baseline characteristic vector is
\begin{equation}
  \theta_i=(a_i,k_i).
...
\(H_C(z)\), where \(z\) is a generic supplier-efficiency value.  The supplier's capacity control and cost function are introduced
...
latter is measured on the same \(\mathsf K\) scale as \(k_i\). Let
\(F\) denote the exogenous joint distribution of project characteristics. Define
\begin{equation}
  \omega=(q,m)\sim F(q,m),
...
\(g\in\{O,\mathrm{Inc}\}\) be an exogenous novelty classifier.  Here \(O\) denotes original/breakthrough-oriented projects and
\(\mathrm{Inc}\) denotes incremental/improved projects. The latter label
is distinct from the internal-manufacturing route label defined below.  Let
...
advancement control for each class.
...
finite post-MAH burden is denoted by \(\bar\tau_E\geq0\). The baseline
representation is
\begin{equation}
  \tau_E(0)=+\infty,
  \qquad
...
\(\tau_E(0)=+\infty\), which makes the entrusted route unavailable when values are assembled.  Thus the common label domain is not a second policy channel.
...
\emph{project-development / advancement intensity}: effort that advances
viable drug projects to the stage at which commercialization routes are
planned. Upstream scientific research, basic-compound discovery,
patent-generating effort, and patent applications are outside this control.
Let \(x_i^*\) reserve the optimal value of this common control; its
optimization is derived in the project-advancement section.

Research capability converts the control into the expected measure of viable
...
\begin{enumerate}
  ...
  \((q,m)\) and the manufacturing-service price, the project chooses a route
  from the domain in \eqref{eq:p01-route-domain}.  Actual holder--producer
  separation is observed only if the realized route is \(E\), which occurs
  after the Stage~1 choice.
...
not directly lower \(p_m^*\).  Nothing in this section implies an increase in
