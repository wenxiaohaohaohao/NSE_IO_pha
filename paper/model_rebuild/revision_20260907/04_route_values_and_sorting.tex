...
\(W_i^I=-\infty\), so internal production is unavailable. This is an extended-value convention;
we do not evaluate the undefined internal marginal cost below feasibility.
...
\end{equation}
Alongside continuous heterogeneity, impose the no-tie regularity restriction:
for each fixed candidate price and regime, maximizing ties have zero project
probability for almost every developer. Continuity alone does not imply this
restriction. A fixed measurable selection resolves null-set ties. Thus the
choice is single valued almost surely. This is a
...
\end{equation}
The value gain is exactly zero whenever the newly feasible entrusted value
fails to exceed the best pre-reform route. The route is unchanged when the
inequality is strict; equality can affect only a tie selection, a null event
under the stated regularity restriction.

\subsection{Internal--entrusted value gap}
\label{subsec:p04-value-gap}

For fixed \(q,m,M,p_m\) with finite \(\tau_E(M)\) and internal
feasibility, define
\begin{align}
...
\end{align}
On the interior of the internally feasible domain, only the internal technology terms vary
...
\(k^*\), contracting that region. The price derivative is a conditional
...
effect when \(E\) remains below the pre-reform maximum, including when
\(T\) or \(A\) still dominates after reform. Failure of the finite
\(I/E\) crossing condition alone does not imply zero reform gain.
No probabilistic route share is used.
