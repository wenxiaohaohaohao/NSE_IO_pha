...
manufacturing capability \(k_i\) fixed. On the interior of the internally feasible domain,
...
\(k_i<\underline{k}(m)\), internal production is infeasible. As
...
preserved in the limit for each fixed project. Each fixed project eventually
exits the relevant set; without bounded support this need not occur at one
common finite price. The integrable value envelope gives convergence of
expected gain to zero.
...
sufficient condition \(\nu\geq1\). For comparisons including the exact
feasibility endpoint, require right-continuity of feasible internal costs
there; no derivative across the extended-value boundary is taken.
...
positive-measure set. For \(\nu>1\), a strict rise in baseline value
also makes the concavity channel strict when the reform surplus remains
positive; zero surplus cannot generate a strict response. For \(0<\nu<1\), no sign is asserted
...
\paragraph{Economic interpretation.}
Different novelty classes may induce different distributions of manufacturing
requirements, commercial values, and hence success probabilities, costs and
outside options through their \(F_g\) distributions. Only
...
primitive supply, background-demand, no-tie and integrability conditions
of the capacity-market section. That section derives continuous nonincreasing
finite demand and continuous strictly increasing supply, with positive excess
demand at zero and negative excess demand at high price. These aggregate
properties are derived inputs to this proof, not assumed economic conclusions.
...
\paragraph{Zero-effect and boundary cases.}
If study demand is zero at \(p_m^0\), uniqueness gives \(p_m^1=p_m^0\). If supply is
...
probability \(s(q)\) is exogenous and policy invariant. For typed baseline
outcomes write \(s_g(q)=s(q)\): the same kernel enters valuation and realized
outcomes. The optional separate class-specific kernel is not activated.
...
\end{equation}
There is one common \(x_i^*\), not an \(x_{ig}\) control. By the mixture
identity and the common realization kernel,
\(\sum_g Y_{ig}^{\mathrm{ret}}=Y_i^{\mathrm{ret}}\).
...
unchanged. Since old route values remain available and price invariant, a
previously retained project can stay on \(I\) or switch to \(E\), but cannot
switch to a strictly better \(T/A\) option after reform. Thus retained
realization mass is weakly increasing (away from null ties). Together with
Proposition 5 this yields weakly nonnegative retained-outcome changes, while
strict increases and cross-class rankings need further conditions. A class-specific outcome can have zero change while the other
changes, for example when the unchanged class has zero retained realization
mass in both regimes. Zero conditional value gain alone is not sufficient:
if common advancement increases and a class has positive pre-reform retained
realization mass, that class also receives the advancement component. The
baseline allows
