...
is project-development / advancement intensity: it advances viable drug
projects to a commercialization-relevant route-planning stage. It excludes
patent applications, patent-generating effort, basic research, and upstream
scientific discovery. There is no class-specific control \(x_{ig}\).
...
Let \(\kappa>0\) be the cost scale and \(\nu>0\) its curvature
parameter. The advancement-cost function is
\begin{equation}
  C_X(x_i)
  =
  \frac{\kappa}{1+\nu}x_i^{1+\nu},
...
characteristics, especially manufacturing capability \(k_i\). Because the
route-value maximum contains abandonment with value zero,
\(W_i(q,m;M,p_m)\geq0\). Assume measurable route values and finite expected optimized value. A
sufficient condition is integrability of the sum of nonnegative gross
operating values of feasible routes and the positive part of \(T\). Then
\[
  0\leq\Omega_i(M,p_m)<+\infty.
\]
The expectation averages the optimized deterministic project value.
...
\[
  W_i(q,m;1,p_m)\geq W_i(q,m;0,p_m).
\]
...
of projects for which the newly feasible entrusted route strictly improves
the pre-reform maximum. Otherwise both expected value and advancement can
have a zero reform effect.
...
For an appendix-only value decomposition, let \(B_i\) denote the continuation-value
...
research and development controls, no binding financial constraint, no
patent outcome, no novelty-class-specific control, and no CMO market-clearing
condition. Project advancement is ex ante; route choice and observed
