...
On the interior of the internally feasible domain,
\begin{equation}
  \Delta_{IE,k}
  =
...
For this local formula, assume differentiability and an integrable envelope
for local difference quotients, permitting differentiation under the
developer integral. Then
\begin{equation}
...
exist, the global weak decrease follows from the product of nonnegative monotone
responses. The no-tie and integrability assumptions ensure continuity,
not necessarily classical differentiability.
...
price. Hold every other primitive fixed. Assume continuously differentiable
aggregates with finite derivatives locally and the derivative envelopes just
stated. Supplier integration gives \(S_{m,p}=\int 1/\Psi_{ss}\,dH_C>0\),
while demand monotonicity gives \(D_{m,p}\leq0\). Differentiating
...
price derivative uses the additional local differentiability and derivative
envelopes; strict monotonicity alone would not imply a nonzero slope. The
derived positive supply derivative makes its denominator strictly positive. It is zero when the perturbation changes
...
study cohort creates positive demand and an advancement response no larger than the fixed-price benchmark. A
cross-market ranking by supply elasticity alone requires additional
comparability conditions on demand and the pre-reform benchmark; it does
not follow from this within-market comparison.
...
\(\rho_gF_g\), \(s_g=s\), and the common \(x_i\), not a class-specific
control. Zero conditional class surplus alone is not a zero-output condition
when common advancement rises and that class retains positive realized mass.
...
On the feasible interior, \(\Delta_{IE,k}>0\), so increasing capability weakly
...
\paragraph{Insufficient internal capability.}
...
\]
For each fixed project, \(E\) eventually ceases to be optimal; no common
finite exclusion price is asserted for unbounded project support. Thus
\begin{equation}
  \lim_{p_m\to\infty}
  W_i(q,m;1,p_m)
  =
