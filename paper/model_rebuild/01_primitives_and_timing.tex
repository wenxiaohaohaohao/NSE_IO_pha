% Phase 1 only: definitions and timing. No payoff or optimization derivation.
...
characteristics follow the exogenous distribution \(H(a,k)\).  Developer \(i\)
has research capability \(a_i>0\), measured in \(\mathsf P/\mathsf X\), and
internal manufacturing capability \(k_i>0\), measured in \(\mathsf K\).  Its
complete permanent baseline characteristic vector is
\begin{equation}
  \theta_i=(a_i,k_i).
...
project's route control, its institutional domain is
\begin{equation}
  r_i\in
  \begin{cases}
    \{I,T,A\}, & M=0,\\
...
\(r_i^*(q,m;M,p_m)\), but route values and the argmax are deferred to
Phase~4.
...
value.  These objects are semantically reserved here: route values are built in
Phases~2--4, the advancement optimization is solved in Phase~5, and the CMO
fixed point is closed in Phase~6.
...
\begin{enumerate}
  ...
  realized and publicly observed.  It changes only the barrier or legal
  availability of route \(E\) in
  \eqref{eq:p01-institutional-wedge}--\eqref{eq:p01-route-domain}.
...
The complete exact-category registry, dimensions, and forbidden
interpretations are recorded in
\texttt{paper/model\_notes/01\_symbols\_and\_objects.md}; the initial equation
graph and deferred fixed point are recorded in
\texttt{paper/model\_notes/02\_equation\_dependency\_map.md}.
