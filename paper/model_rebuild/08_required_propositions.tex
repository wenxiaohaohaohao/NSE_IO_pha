% Phase 8 only: six substantive propositions and one novelty corollary.
...
use the Phase~3 technology signs and the Phase~4 endpoint-crossing conditions.
...
\paragraph{Proof.}
Phase~2 gives \(R_c<0\). Phase~3 gives \(c_{I,k}<0\) and \(F_{I,k}<0\).
Both terms on the right side of \eqref{eq:p08-sorting-slope} are therefore
strictly positive. Continuity and endpoint crossing give existence by the
intermediate value theorem; strict monotonicity gives uniqueness. The sign of
...
option. Route values retain the Phase~4 definitions.
...
\paragraph{Assumptions and fixed objects.}
Fix \(M\) and all exogenous distributions and primitives. Use the Phase~6
conditions: continuous weakly decreasing finite demand, continuous strictly
...
\end{equation}
This is the Phase~1 off-equilibrium technology
\(\lambda_i^{\mathrm{plan}}=a_ix_i\) evaluated at the Phase~5 optimizer.
...
\subsection{Phase 8 scope conclusion}
\label{subsec:p08-scope}

The six propositions and the novelty corollary exhaust the authorized Phase 8
substantive results. They add no market, no entry margin, no welfare object, no
