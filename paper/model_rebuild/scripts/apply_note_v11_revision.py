...
> **Effective specification:** original execution note + amendment v1.1, under the user's current strict execution rules.  
> **Authority:** current user instruction > amendment v1.1 > unchanged portions of the original note. Repository v1.2 and September 3/4 instructions are historical evidence only for this revision.  
> **Method:** targeted amendments to the immutable note; no restart or wholesale rewrite.  
> **Continuation:** the September 1 package at 8a99cc3 is the selected manuscript baseline. Historical Phases 1--18 are retained; current revalidation and approval are tracked separately.  
> **Gates:** every phase audit pauses for explicit user approval; historical standing authorization does not apply to this revision. The full derivation draft belongs to Phase 15 and requires approval before Phase 16. Manuscript changes require the Phase 18 gate.  
> **Counts:** RL-01--RL-28 and SUCCESS-01--SUCCESS-13 incorporate amendment C and V.
"""
...
precision_block = r"""
### Precision on the Innovation Object (Amendment v1.1)

The baseline control is

\[
\boxed{x_i=\text{innovation-project advancement / development effort},}
\]

or, briefly, project-development intensity. It advances viable drug projects toward the route-planning / commercialization-relevant development stage:

\[
\boxed{\lambda_i^{plan}=a_ix_i.}
\]

Upstream scientific research, basic-compound discovery and patent-generating effort are outside the baseline. Potential empirical counterparts include IND/application activity, early-stage clinical development and first-stage clinical trials. They are measurement candidates rather than assumed available data. The model concerns the value of advancing viable projects toward commercialization; it does not directly model upstream discovery or imply rising patent applications.
"""
insert_before(
...
replace_once(
    r"\text{ex ante R\&D}",
    r"\text{ex ante project development / advancement}",
...
\]"""
new_stage_one = r"""### Stage 1: Project-development / advancement investment
...
Interpret \(\Lambda_i^{plan}\) as arrival/advancement of viable projects to the commercialization-relevant development stage. It is development-stage advancement, not patent generation. Potential data counterparts such as IND/application activity or early clinical milestones require a separate measurement audit.
"""
...
replace_once(r"\text{R\&D effort}", r"\text{project development / advancement}", "AMD11-K-HIERARCHY")
...
| \(x_i\) | project-development / advancement intensity | not directly observed; candidate development-stage activity requires validation | development-stage activity, not patent applications |
...
# Amendment N.
crosswalk_rows = r"""
| \(x_i\) described as generic R&D effort | reinterpret as project-development / advancement intensity |
...
In Sections 3.1 and 3.3, use “project-development / advancement intensity” rather than generic “R&D effort,” unless the narrower meaning is explicitly defined. The appendix must include a short subsection, “Research versus Development: Boundary of the Baseline,” explaining that MAH changes downstream commercialization options, that \(x_i\) models advancement of viable drug projects and excludes patent-generating upstream research, and that Shi Gu (2024) shows these margins can differ.
"""
...
| 18 Manuscript integration | NOT STARTED | manuscript and final build | — | No |"""
...
amendment_action = r"""
# 29A. Amendment v1.1 Application Checkpoint

Produce the amendment status report with sections modified, new red lines, terminology, Phase 5, propositions, empirical mapping, extension boundary, conflicts with completed work, affected files, and P0 issues. Pause for explicit approval after the audit. Resume the existing work; do not restart the note or discard completed derivations.

Already-completed work that interprets x_i as generic upstream research or claims a mandatory positive original-innovation response must be repaired and re-audited before any new derivation phase. This revision continues the September 1 completed derivation stack; its current revalidation states are separate from historical approvals.
"""
...
# Complete the required FOC interpretation, and repair inherited display placement.
text = text.replace("C_R'(x_i)", "C_X'(x_i)")
text = text.replace("Quadratic R&D cost", "Quadratic project-advancement cost")
text = text.replace("### Quadratic R&D", "### Quadratic project advancement")
for payload in (prop3_boundary, planning_interpretation, type_outcomes):
    text = text.replace("\n\n" + payload.rstrip() + "\n\\]", "\n\\]\n\n" + payload.rstrip())
text = text.replace("Check SOC and corner cases.", "Check SOC and corner cases. The marginal cost of advancing viable drug projects equals the marginal increase in route-planning-stage project arrival multiplied by the expected commercialization value of such a project.")
# Claims in a specification are requirements to verify, not fresh empirical verification.
text = text.replace("The verified SSRN abstract reports", "Amendment v1.1 reports")
text = text.replace("The SSRN abstract currently verifies only the core research/development boundary.", "Use the preserved Shi Gu evidence ledger as historical evidence; revalidate source version and page/table support before presenting final empirical claims.")
text = text.replace("Detailed items remain `UNVERIFIED` until checked in the formal paper PDF.", "Detailed items require page/table verification in the formal paper PDF; do not replace historical verified evidence with an invented fresh verification.")
text = text.replace("Start only with **Phase 1**.", "For a first-ever execution only, start with **Phase 1**. For this resumed revision, retain completed work and follow current revalidation gates rather than restarting the original note.")
text = text.replace("demand therefore includes both intensive R&D response", "demand therefore includes both intensive project-advancement response")
text = text.replace("The more detailed claims listed in the Phase 16/18 empirical checkpoint remain `UNVERIFIED` until the paper PDF is checked by page and table.", "The detailed claims require page/table evidence; preserve and revalidate the historical evidence ledger before final integration.")
