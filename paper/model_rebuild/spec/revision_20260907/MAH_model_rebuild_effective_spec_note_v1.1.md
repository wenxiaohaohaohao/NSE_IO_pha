...
\text{project value}
\rightarrow
\text{ex ante project development / advancement}
\rightarrow
...
- **Shi Gu (2024), “Production Outsourcing and Innovation: Evidence from China's Pharmaceutical Industry”:**
  empirical boundary and benchmark. Amendment v1.1 reports increased clinical development among firms without production facilities, a patent-application decline among financially constrained firms concentrated in lower-value patents, and a stronger incremental-development response. The detailed claims require page/table evidence; preserve and revalidate the historical evidence ledger before final integration. Shi Gu is not an additional mother model, and its research-versus-development financial-constraint model must not enter the baseline.
...
Quadratic project-advancement cost is the special case \(\nu=1\).
...
Check SOC and corner cases. The marginal cost of advancing viable drug projects equals the marginal increase in route-planning-stage project arrival multiplied by the expected commercialization value of such a project.
...
- demand therefore includes both intensive project-advancement response and route-selection response;
...
\[
\text{project development / advancement}
\]
...
Use the preserved Shi Gu evidence ledger as historical evidence; revalidate source version and page/table support before presenting final empirical claims. Detailed items require page/table verification in the formal paper PDF; do not replace historical verified evidence with an invented fresh verification. `UNVERIFIED` does not block Phase 1--15 derivations, but it blocks Phase 16 empirical-boundary prose and Phase 18 manuscript integration. For every verified fact, state whether it is predicted by baseline, outside baseline, a heterogeneity test, or an extension motivation.
...
For a first-ever execution only, start with **Phase 1**. For this resumed revision, retain completed work and follow current revalidation gates rather than restarting the original note.
