...
characteristics, especially manufacturing capability \(k_i\) and financing
capacity \(\ell_i\). Because the
route-value maximum contains abandonment with value zero,
\(\widetilde W_i(q,m;M,p_m,\ell_i)\geq0\). Under the stated integrability condition,
\[
...
Conditional on \(M,p_m,a_i,k_i,\ell_i\), the expected value
...
\[
  ...
  \geq \widetilde W_i(q,m;0,p_m,\ell_i).
\]
...
of projects in \(\mathcal C_i(p_m,\ell_i)\), where entrusted production is
financeable and strictly improves the pre-reform maximum. Otherwise both
...
\(a_ix_i\) or \(C_X(x_i)\). Financing capacity also affects advancement only
through the set of financeable commercialization routes. It does not enter the
advancement FOC directly.

Within a fixed regime, higher \(\ell_i\) weakly expands the feasible route set,
so \(\Omega_i\) and \(x_i^*\) are weakly increasing in the finite-comparison
sense. The binary reform response
\(\Delta x_i(\ell_i)=x_i^*(1,p_m,\ell_i)-x_i^*(0,p_m,\ell_i)\) has no global
monotonic sign in \(\ell_i\): financing first makes \(E\) feasible and may
later restore \(I\) as an old option. Derivatives at the financing thresholds
need not exist.
...
The baseline contains one common \(x_i\), one cost function \(C_X\), one
expected optimized project value \(\Omega_i\), and predetermined
commercialization financing capacity \(\ell_i\). It contains no separate
...
block. Project advancement is ex ante; financing feasibility, route choice,
and observed holder--producer separation are downstream.
