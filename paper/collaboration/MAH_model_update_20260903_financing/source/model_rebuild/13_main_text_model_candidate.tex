...
\begin{abstract}
This candidate is the restrained main-text representation of the current
financing-adjusted derivation. The MAH reform makes retained entrusted manufacturing
...
adds predetermined commercialization financing capacity only as a route
feasibility margin. MAH changes organizational technology rather than finance
supply. The baseline contains neither patent-generating research nor a fixed
ranking between original and incremental project responses.
\end{abstract}
...
research capability \(a_i>0\), internal manufacturing capability \(k_i>0\),
and commercialization financing capacity \(\ell_i>0\), distributed according
to the joint distribution \(H(a,k,\ell)\). A viable project that reaches
...
draw \((q,m)\), check route financing requirements, and choose a route. Fourth,
...
\(F_{I,m}>0\) and \(F_{I,k}<0\). Its route value is
\begin{equation}
  W_i^I(q,m)
  =
...
\end{equation}
It requires a liquidity commitment \(J_I(m,k_i)>0\), with
\(J_{I,m}>0\) and \(J_{I,k}<0\), and is financially feasible if and only if
\(J_I(m,k_i)\leq\ell_i\).
...
\end{equation}
It requires \(J_E(m)>0\), with \(J_E'(m)>0\), and is financially feasible if
and only if \(J_E(m)\leq\ell_i\). The \(J\)'s are liquidity requirements, not
additional costs. Each monetary cost enters once. The transfer and abandonment values are
\begin{equation}
  W^T(q,m)=T(q,m),
  \qquad
  W^A=0.
...
Define \(\widetilde W_i^I=W_i^I\) when \(J_I\leq\ell_i\) and
\(-\infty\) otherwise, and define
\(\widetilde W_i^E=W_i^E\) when \(J_E\leq\ell_i\) and \(-\infty\) otherwise.
MAH changes none of \(\ell_i,J_I,J_E\).
...
\begin{equation}
  ...
  \label{eq:p16m-gap}
\end{equation}
Holding \((q,m,M,p_m)\) fixed conditional on both retained routes being
financeable,
\[
  \Delta_{IE,k}
  =
...
\]
When the endpoint crossing conditions hold on the common-financeability set, a unique conditional cutoff
...
\begin{equation}
  ...
  &=
  \int a_ix_i^*(M,p_m)\chi_i^E(p_m;M,\ell_i)\,dH(a,k,\ell),\\
  D_m(p_m;M)
...
The reform has a zero effect whenever \(E\) is not financeable or its value
does not exceed the best financeable old route. Under the local ordering
\(J_E<J_I\), the interval \(J_E\leq\ell_i<J_I\) is the financing corridor in
which retained outsourcing is financeable but internalization is not. The
reform response need not be globally monotone in \(\ell_i\), because higher
financing also restores internalization as an old option. MAH does not change
\(a_i\), \(\ell_i\), project quality, or the advancement-cost function.
