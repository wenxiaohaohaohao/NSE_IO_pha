...
capacity units. Commercialization financing capacity and requirements are
one-time monetary commitments measured in \(\mathsf C\), but they are stocks
of fundable liquidity rather than additional payoff costs.

There is a continuum \(\mathcal I\) of original-drug developers.  Developer
characteristics follow the exogenous joint distribution \(H(a,k,\ell)\), with
...
also has predetermined commercialization-stage financing capacity
\(\ell_i>0\), measured in \(\mathsf C\). Financing capacity is the largest
up-front organizational commitment the developer can fund; it is neither
research productivity nor manufacturing know-how. Its
...
\(r_i^*(q,m;M,p_m,\ell_i)\), but route values and the argmax are defined in the
...
\begin{enumerate}
  ...
  observes \((a_i,k_i,\ell_i,M)\), anticipates the market-consistent service price
  ...
  \((q,m)\) and the manufacturing-service price, the developer checks each
  retained route's up-front financing requirement against \(\ell_i\), then
  chooses a route from the domain in \eqref{eq:p01-route-domain}. There is no
  borrowing or separate financing decision. Actual holder--producer
...
In particular, \(M\) does not directly change \(a_i\), \(k_i\), \(\ell_i\), \(z_j\),
\(q\), \(m\), \(F\), \(\rho_g\), \(F_g\), \(s(q)\), or \(s_g(q)\).  It does
not directly change route financing requirements and does not directly lower
\(p_m^*\). Nothing in this section implies an increase in
