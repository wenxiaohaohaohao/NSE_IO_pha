...
The baseline remains intentionally narrow: a partial-equilibrium dynamic route-choice model with one CMO service-market closure. The appendix does not convert the paper into a product-market GE model, an R\&D input-market GE model, a complete free-entry and exit model, a welfare model, or a full bargaining model.
\section{Model}
...
Endogenous route and R\&D objects & $P_i(r\mid M,p_m^*)$, $\Gamma_i(M,p_m^*)$, $x_i^*(M)$, $\lambda_i^{lat,*}(M)$, $\lambda_i^{obs,*}(M)$ & Determined sequentially by route payoffs, logit route choice, CMO service-market clearing, and the R\&D FOC & Observed output is downstream of latent opportunity arrival \\
CMO market object & $p_m^*(M)$ & Determined by $D_m(p_m;M)=S_m(p_m)$ & Only CMO services are cleared; no product-market or R\&D-input GE \\
Composite realization objects & $\zeta_i^E(M,p_m)$, $\bar R_i^E$, route-realization wedge & Compressed objects linking latent opportunities to implementation and observed outcomes & Route-use and count moments discipline composites, not all primitives \\
...
\begin{equation}
    ...
    \label{eq:logit_prob}
\end{equation}
When the CMO service price needs to be shown explicitly, $P_i(r\mid M_{t+1},p^*_{m,t+1})$ denotes the same logit probability evaluated using route payoffs at the market-clearing service price. For routes other than retained entrusted production, $\zeta_i^r(M,p_m)$ may be independent of $p_m$.
...
\begin{equation}
    ...
    a_i x^*_{it}
    \sum_{r\in\calR_i(M_{t+1})}
    P_i(r\mid M_{t+1},p^*_{m,t+1})\zeta_i^r(M_{t+1},p^*_{m,t+1}).
    \label{eq:lambdaobs}
\end{equation}
...
\begin{equation}
    ...
    =
    a_i x^*_{it}
    P_i(E\mid M_{t+1},p^*_{m,t+1})
    \zeta_i^E(M_{t+1},p^*_{m,t+1}).
    \label{eq:lambdaobsE}
\end{equation}
...
When $E\in\calR_i(M)$, define the entrusted-route marginal payoff effect of a change in the MAH-reducible friction, holding the market-clearing service price fixed. Assume $\bar R_i^E+v\ge0$ and $\partial\zeta_i^E/\partial\tau_i^E\le0$, so a higher entrusted-route friction does not improve route realization. Then
\begin{equation}
    ...
    \label{eq:dGEdtau}
\end{equation}
If $E\notin\calR_i(M)$, set $P_i(E\mid M,p_m^*(M))=0$. The derivative of the inclusive value is
\begin{equation}
...
\end{align}
The response is stronger for high-$a_i$ firms and for firms with high $P_i(E\mid M,p_m^*(M))$.
\end{proposition}
...
\[
    \lambda_i^{obs,E}(M)
    =
    a_i x_i^*(M)P_i(E\mid M,p_m^*(M))\zeta_i^E(M,p_m^*(M)).
\]
...
\begin{equation}
    \zeta_i^E(M,p_m^*(M))\left[\bar R_i^E+v\right]
...
\begin{equation}
    ...
    \frac{\beta a^2}{\kappa}
    \Gamma^M(\theta)
    \sum_{r\in\calR_i(M)}P_i(r\mid M,p_m^*(M))\zeta_i^r(M,p_m^*(M))
    \,\dd H_I(\theta,n).
    \label{eq:LambdaIobs}
\end{equation}
...
For any fixed distribution of incumbent firm characteristics, a net increase in $\Gamma^M(\theta)$ weakly raises model-implied aggregate expected original-drug opportunities from incumbent firms. Conditional on the entrusted route being feasible and holding the CMO price feedback fixed,
\begin{equation}
    \frac{\partial \Lambda_I^{lat}(1)}{\partial\tau^E}\le0,
    \label{eq:agg_prop_derivative}
\end{equation}
so $\dd\tau^E<0$ implies $\dd\Lambda_I^{lat}\ge0$ through the friction channel. With endogenous CMO pricing, the same conclusion requires the price response in $p_m^*(M)$ not to offset the entrusted-route value gain. The increase is strict if a positive-measure set of incumbents has $a>0$, positive inclusive-value response, and positive entrusted-route probability. If a credible firm-year entry panel is available and the optional entry module is activated, the same sign logic can be extended to potential entrants through $F_e$, but that is not a baseline empirical conclusion.
\end{corollary}
...
Effective entrusted-route wedge containing $\tau^E$ & MAH-reducible policy, contractual, and regulatory friction embedded in the retained external route payoff & Holder--producer split among comparable original-drug records; post-MAH exposure-gradient in split shares & Enters a composite route-use wedge; does not separately identify legal cost, search cost, CMO price, or holder burden \\
$\zeta_i^E$ & Composite realization probability for the retained entrusted route & Application-to-approval conversion, approval-to-launch conversion, launch or realization conversion; split shares discipline it only jointly with route choice & Distinguishes latent arrival from observed realization; not separately identified from $\bar R^E$ or route choice without additional data \\
...
\begin{equation}
    ...
    \sum_{r\in\calR_i(0)}P_i(r\mid0,p_m^*(0))\zeta_i^r(0,p_m^*(0))
    \right].
...
Original-drug counts & Baseline original-drug approvals, realized products, or applications & $\bar\lambda_0^{O,obs}=\E[\frac{\beta a^2}{\kappa}\Gamma^0(\theta)\sum_rP_i(r\mid0,p_m^*(0))\zeta_i^r(0,p_m^*(0))]$ & Composite innovation-arrival scale plus realization mapping \\
...
&\Rightarrow\ x^*
\text{ and }\lambda^{lat,*}\text{ respond through }\Gamma,\\
&\Rightarrow\ \lambda^{obs,*}
\text{ may rise through route use and realization},\\
&\Rightarrow\ \Lambda^{lat,agg}
...
This equation does not say that the route-use share equals $\tau^E$, $\mu^E$, or $\zeta^E$. It says that the route-use share, through a retained-product route-choice equation, disciplines the relative attractiveness of the entrusted route. Without additional data on internal production costs, direct contract-manufacturing costs, quality agreements, audit costs, contract terms, firm-level regulatory execution, and application-to-approval conversion, the calibration disciplines only a composite route-realization wedge relative to the route-choice scale.

\subsection{When a Pre--Post Log-Odds Moment Is Valid}

A pre--post log-odds moment is valid only if the pre-reform data contain a legally and economically comparable entrusted-retention route. Under that condition, and if internal-production costs, CMO prices, residual holder burdens, realization values, and sample composition are stable across the comparison or handled through comparable samples and controls, one can write
\[
...
\[
    ...
    (G_{pre}^E-G_{pre}^I).
\]
The object $\Delta\omega_E$ is a change in the relative entrusted-route payoff wedge, not the primitive change in $\tau^E$ alone. It equals $\tau_{pre}^E-\tau_{post}^E$ only under a restrictive normalization that holds CMO prices, residual holder burden, internal production cost, realization probability, route-specific realized return, and retained continuation value fixed. This is not the baseline interpretation when the MAH-style entrusted route was institutionally unavailable before the reform.
