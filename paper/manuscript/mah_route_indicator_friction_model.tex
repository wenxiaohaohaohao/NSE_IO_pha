...
The object $\tau^E$ is deliberately route-specific. It is not the market return $R$, the research productivity parameter $a_i$, the R\&D cost parameter $\kappa$, or the direct payment for entrusted manufacturing services $p_m$. It captures the governance, compliance, monitoring, documentation, and cross-firm coordination burden of retaining authorization while using a qualified producer. A fall in $\tau^E$ means that the external route becomes more legally supported, contractually structured, and regulatorily intelligible; it does not mean that the holder has no responsibility or that physical manufacturing becomes free.
...
\begin{align}
    ...
    b^E_{i,t+1} &= R_{t+1}-p_{m,t+1}-\tau^E_{i,t+1}(M_{t+1}),\label{eq:bE}\\
...
R_{t+1}-p_{m,t+1}-\tau^E_{i,t+1}+v_{t+1},
S_i-\tau_i^T,
0
\right\}.
\label{eq:Gamma_post}
...
For stationary comparative statics, hold the market-return background and the contract-manufacturing service price fixed and set $R_t=R$, $p_{m,t}=p_m$, $M_t=M$, $\tau^E_{it}=\tau^E$, $v_t=v$, and $A_t=A$. From equation \eqref{eq:vrecursion},
\begin{equation}
...
The comparative statics hold the reduced-form market-return shifter fixed. They have two components. First, the MAH indicator adds the entrusted-production route. Second, conditional on the route being available, the continuous friction $\tau_i^E$ affects the value of that route. The derivative-based results are with respect to $\tau_i^E$.

\subsection{Entrusted-route relevance region}

It is useful to separate the entrusted route from all non-entrusted outside options. Define the best non-entrusted route value as
\begin{equation}
    B_i(k_i,S_i,\tau_i^T)
    =
    \max\left\{
...
\end{equation}
and define the entrusted-route value, conditional on route availability, as
\begin{equation}
    E_i(\tau_i^E)=R-p_m-\tau_i^E+v.
...
\begin{equation}
    ...
    \qquad
    \Gamma_i^1(k_i,\tau_i^E)=\max\left\{B_i(k_i,S_i,\tau_i^T),E_i(\tau_i^E)\right\}.
    \label{eq:Gamma_relevance}
\end{equation}
The entrusted route is relevant for firm $i$ when
\begin{equation}
    E_i(\tau_i^E)>B_i(k_i,S_i,\tau_i^T).
    \label{eq:entrusted_relevant_region}
\end{equation}
This condition gives the economic content of heterogeneity. A firm with weaker internal manufacturing capability has a higher $C^I(k_i)$ and therefore a weaker internal-production outside option. A firm with a stronger non-retained transfer or sale option has a higher $S_i-\tau_i^T$ and may not need the entrusted route. Research ability $a_i$ does not determine route relevance by itself, but once the route raises $\Gamma_i$, it scales the R\&D and count responses through equations \eqref{eq:xstationary} and \eqref{eq:lambdastationary}.

\begin{lemma}[Strict response region]
\label{lem:strict_response_region}
Consider a fall in entrusted-production friction from $\tau_H$ to $\tau_L$, with $\tau_L<\tau_H$, conditional on the entrusted route being feasible. The friction reduction strictly raises opportunity value if and only if
\begin{equation}
    E_i(\tau_L)>B_i(k_i,S_i,\tau_i^T).
    \label{eq:strict_region}
\end{equation}
When this condition holds,
\begin{equation}
    \Delta x_i^*
    =
    \frac{\beta a_i}{\kappa}\Delta\Gamma_i,
    \qquad
    \Delta\lambda_i^*
    =
    \frac{\beta a_i^2}{\kappa}\Delta\Gamma_i,
    \label{eq:response_scaling}
\end{equation}
where $\Delta\Gamma_i=\Gamma_i^1(k_i,\tau_L)-\Gamma_i^1(k_i,\tau_H)>0$.
\end{lemma}

\begin{proof}
By equation \eqref{eq:Gamma_relevance}, the post-MAH opportunity value is the maximum of the best non-entrusted value $B_i$ and the entrusted-route value $E_i(\tau_i^E)$. Since $\tau_L<\tau_H$, $E_i(\tau_L)>E_i(\tau_H)$. The difference $\max\{B_i,E_i(\tau_L)\}-\max\{B_i,E_i(\tau_H)\}$ is strictly positive exactly when the lower-friction entrusted route exceeds $B_i$. The response formulas follow by subtracting equations \eqref{eq:xstationary} and \eqref{eq:lambdastationary} across the two friction levels.
\end{proof}
...
\begin{proof}
Add and subtract $\Gamma_i^1(k_i,\bar\tau_i^E)$ from $\Gamma_i^1(k_i,\bar\tau_i^E-\Delta\tau_i)-\Gamma_i^0(k_i)$. Lemma \ref{lem:route_set_expansion} gives nonnegativity of the route-set term. Since $\bar\tau_i^E-\Delta\tau_i\le\bar\tau_i^E$, Lemma \ref{lem:dGamma_dtau} gives nonnegativity of the friction-reduction term.
\end{proof}

These results are private option-value results. They do not imply that social welfare necessarily rises, that entrusted production is always chosen, or that every firm responds. Route-set expansion weakly raises the value of a successful opportunity because the firm maximizes over a larger feasible set. Lemma \ref{lem:strict_response_region} shows that strict response is concentrated among firms for which the lower-friction entrusted route beats the best non-entrusted option. The model therefore predicts larger realized responses when the firm has high research ability, weak internal manufacturing capability, and no dominant non-retained transfer or sale option.
...
For any fixed joint distribution of research ability and manufacturing ability, and for any fixed conditional entry-cost distribution, MAH weakly raises model-implied aggregate expected original-drug opportunities through route-set expansion and friction reduction. Conditional on the entrusted route being feasible,
\begin{equation}
    \frac{\partial \Lambda^{agg}(1)}{\partial\tau^E}\le0,
...
\begin{equation}
    ...
    =
    \frac{C^I(k)-p_{m,t}-\tau_t^E}{\sigma_r},
    \label{eq:logodds}
\end{equation}
...
The model gives a simple mechanism and a disciplined calibration blueprint. MAH adds the regulated route through which an innovating holder can retain marketing authorization while entrusting production to a qualified manufacturer. Conditional on the route being available and holding the reduced-form market-return background fixed, a lower entrusted-production friction raises the private route-option value $\Gamma$ for firms for which the entrusted route is relevant. Because expected next-period route value enters the Bellman, current R\&D intensity rises in the model. Since the expected number of new original-drug opportunities is produced by $\lambda=ax$, higher R\&D intensity raises expected original-drug counts. The calibration blueprint maps this mechanism to approval-side realized-product moments while preserving the boundary between latent opportunities, retained certificates, route-use proxies, demand-side controls, and realized approvals. The paper does not require project-level R\&D investment, observed project-specific success probabilities, a complete causal design, product-level demand estimation, or a full equilibrium model of contract manufacturing.
...
This institutional structure explains why the entrusted route is economically subtle. MAH did not create a zero-cost outsourcing technology. A research-originating holder cannot simply hand off production and walk away. The external route remains costly, but the cost is attached to a more standardized and legally recognized governance structure rather than to an ad hoc or weakly enforceable arrangement.

\subsection{What the entrusted-route friction is not}

The institutional record also clarifies what $\tau^E$ is not.
...
\textbf{Not a generic fixed-cost reduction for all firms.} MAH does not simply relax every firm's operating burden. The post-law architecture may impose more explicit documentation, auditing, and quality-system obligations on the holder. What changes is that these obligations are attached to a legally recognized and standardized external route, so that the net friction of using that route can fall relative to the pre-reform environment. In other words, MAH can preserve or sharpen holder-side responsibility while still lowering the effective governance cost of external implementation by making the arrangement legally supported, contractually structured, and regulatorily intelligible.

This is why the model treats $\tau^E$ as a route-specific friction. It is distinct from the direct payment to a qualified manufacturer, from the market value of a successful drug, and from the cost of generating the scientific idea. The comparative statics in the model should therefore be read as statements about the private option value of a legally recognized entrusted-production route, not as statements about demand, science, or a generic deregulation shock.
...
This equation does not say that the route-use share equals $\tau^E$. It says that the route-use share, through a retained-product route-choice equation, disciplines the relative attractiveness of the entrusted route. Without additional data on internal production costs, direct contract-manufacturing costs, quality agreements, audit costs, contract terms, and firm-level regulatory execution, the calibration recovers only a composite effective wedge relative to the route-choice scale.

\subsection{When a Pre--Post Log-Odds Moment Is Valid}

A pre--post log-odds moment is valid only if the pre-reform data contain a legally and economically comparable entrusted-retention route. Under that condition, and if $C^I(k)$ and $p_m$ are stable across the comparison or handled through comparable samples and controls, one can write
\[
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{What can be claimed:} route-use proxies and realized original-drug outcomes can discipline whether observed patterns are consistent with route-set expansion and a lower effective entrusted-production wedge.
