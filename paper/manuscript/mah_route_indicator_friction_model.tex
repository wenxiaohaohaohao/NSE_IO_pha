...
\begin{equation}
    ...
    \label{eq:routeset}
\end{equation}
where $I$ is internal production, $E$ is entrusted production while retaining MAH, $T$ is a non-retained outside option such as transfer, sale, or out-licensing that removes the opportunity from the innovating firm's retained stock, and $A$ is abandonment or delay.
...
\begin{equation}
    ...
    \label{eq:CIderiv}
\end{equation}
so higher manufacturing ability lowers internal production cost. $C^E$ is the direct cost or market payment associated with entrusted production. The separate term $\tau^E_{i,t+1}$ is the institutional friction of retaining authorization while using a qualified entrusted manufacturer, including the governance and compliance burden specific to that holder-producer assignment. $S_i-\tau_i^T$ is the net value of the non-retained outside option.
...
A pre--post log-odds moment is valid only if the pre-reform data contain a legally and economically comparable entrusted-retention route. Under that condition, and if $C^I(k)$ and $C^E$ are stable across the comparison or handled through comparable samples and controls, one can write
\[
...
\[
    ...
    \eta_0+\eta_1 Exposure_g+u_g,
\]
where $g$ indexes a region, dosage-form segment, therapeutic class, or firm type. This disciplines relative differences in the effective entrusted-route wedge without imposing a nonexistent pre-reform route-use rate.
