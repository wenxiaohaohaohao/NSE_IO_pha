...
\section{Single-Market Demand Background}

The baseline model keeps a single reduced-form market-return shifter rather than introducing a therapeutic-area state space. This is a deliberate modeling boundary. The paper studies how MAH changes commercialization options conditional on the expected market return to a successful original-drug opportunity. It does not model different therapeutic markets as separate firm-level innovation problems.

Let $D_t$ denote aggregate demand-side conditions in China's original-drug environment, and let $Z_t$ collect demand-side controls such as reimbursement environment, procurement exposure, disease burden, patient population, and therapeutic-area composition. The market-return shifter is
\begin{equation}
    R_t = R(D_t,Z_t),\qquad R_D(D_t,Z_t)>0.
    \label{eq:single_market_return}
\end{equation}
Equation \eqref{eq:single_market_return} says that stronger demand-side conditions raise the expected commercialization payoff of a successful original-drug opportunity. It does not say that MAH changes demand. In the main comparative statics, $R_t$ is held fixed, and MAH affects innovation through route-set expansion and entrusted-route friction reduction.
...
For stationary comparative statics, hold the market-return background fixed and set $R_t=R$, $M_t=M$, $\tau^E_{it}=\tau^E$, $v_t=v$, and $A_t=A$. From equation \eqref{eq:vrecursion},
\begin{equation}
