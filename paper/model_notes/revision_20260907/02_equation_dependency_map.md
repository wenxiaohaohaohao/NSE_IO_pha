...
Phase status: `CURRENT PHASE 13 REVISION - IN PROGRESS`  
...
paper/model_rebuild/revision_20260907/audit/phase13_equation_catalog.csv. No object in the
...
|---|---|---|---|---|
| `P03-E01` / `eq:p03-internal-cost` | $c_I(m,k_i)>0$ with $c_{I,m}>0$, $c_{I,k}<0$ on the interior of the feasible domain | $m,k_i$ | primitive technology function and shape restrictions | Phase 4 composes $R(q,c_I)$ |
| `P03-E02` / `eq:p03-internal-setup` | $F_I(m,k_i)$ with $F_{I,m}>0$, $F_{I,k}<0$ on the feasible interior and $+\infty$ if $k_i<\underline{k}(m)$ | $m,k_i,\underline{k}$ | primitive setup technology plus feasibility convention | verify the internal-infeasibility limit |
...
| `P04-E05` / `eq:p04-route-choice` | $r_i^*$ is the argmax over the four explicitly listed route values | `P04-E01`--`P04-E04` | deterministic choice | ties have measure zero; route-indexed alias explicitly defined in Phase 7 |
...
| ID / LaTeX label | Object/result | Inputs already defined | Mathematical status | Proof / scope obligation |
|---|---|---|---|---|
| P08-E01 / eq:p08-sorting-slope | $\Delta_{IE,k}>0$ | P04-E07--P04-E08 | inherited derivative inside Proposition 1 | fixed $q,m,M,p_m$ and feasible internal domain |
...
| P08-E06 / eq:p08-advancement-change | closed-form $\Delta x_i$ from $\Omega_i^1=\Omega_i^0+\Delta\Omega_i$ | P05-E08, P08-E05 | derived finite response | strict iff expected gain is positive |
...
| P08-E08 / eq:p08-manufacturing-response | $\Delta x_i$ weakly decreases in $k_i$ under $\nu\geq1$ and inherited monotonicities | P04-E08, P08-E05--P08-E06 | sufficient comparative result | no unconditional sign for $\nu<1$ |
...
| P08-E13 / eq:p08-scarcity-advancement-bounds | $0\leq\Delta x_i^{eq}\leq\Delta x_i^{dir}$ | P05-E08, P08-E12 | monotone transformation | fixed baseline value |
...
| $x_i\equiv$ clinical-development effort | v1.1 says $x_i$ is broader |
