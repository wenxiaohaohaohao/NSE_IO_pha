...
Phase status: `PHASE 1 REVISION - APPROVED`  
...
|---|---|---|---|---|---|
| $\mathcal I$ | aggregate/distributional primitive | continuum of drug developers | index set | Phase 1 | No |
...
| $q$ | exogenous project characteristic | project commercial/scientific value shifter for viable drug projects | $q>0$, dimensionless index | Phase 1 | No; `RL-08` |
...
| $g$ | exogenous project characteristic | empirical novelty classifier: $O$ original/breakthrough-oriented, $\mathrm{Inc}$ incremental/improved; not a state or control | categorical label, unit $1$ | Phase 1 | No |
...
| $x_i$ | control | project-development / advancement intensity chosen before project draws | $x_i\geq0$, units $\mathsf{X}$ | Phase 1 | No direct shift; expected-value channel later |
...
There are 28 active rows. Each has one and only one exact category.
