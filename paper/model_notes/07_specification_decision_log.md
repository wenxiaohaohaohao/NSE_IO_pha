...
| `DEC-POST18-001` | Add predetermined commercialization financing capacity $\ell_i$ and liquidity requirements $J_I,J_E$ to the active baseline; MAH changes only the availability of retained entrusted organization and never financing supply. | User-supplied `MAH_financing_friction_codex_notes.md`, 2026-09-03 | SUPERSEDED by `DEC-POST18-005` |
| `DEC-POST18-002` | Treat $J_I,J_E$ as financeability thresholds, not additional payoff costs; remove the blanket hard low-$k$ internal-infeasibility convention. | Financing-note accounting and nesting requirements | SUPERSEDED AS BASELINE; accounting retained in inactive extension |
| `DEC-POST18-003` | Use a local financing advantage $J_E<J_I$ to characterize the corridor, not a global ordering; within-regime value is weakly increasing in $\ell_i$, while the MAH treatment effect has no global monotone sign. | Financing-note Sections 5, 8, 10 and mathematical closure audit | EXTENSION-ONLY / INACTIVE |
| `DEC-POST18-004` | Supersede and archive the active research--development/patent module. The current model has one common $x_i$ and no patent-production function. | Current user request has priority over prior revision | ACTIVE |
| `DEC-POST18-005` | Recenter the active baseline on commercialization organization with $\theta_i=(a_i,k_i)$; financing capacity and liquidity thresholds are preserved only in an explicitly inactive appendix extension. | User-supplied `MAH_recenter_financing_extension_codex_notes.md`, 2026-09-04 | ACTIVE / CONTROLLING |
| `DEC-POST18-006` | Internal production remains available at every $k_i>0$; weak capability raises finite $c_I$ and $F_I$. No hard positive capability cutoff is part of the baseline. | 2026-09-04 recentering instruction | ACTIVE / CONTROLLING |
| `DEC-POST18-007` | Manufacturing capability is the primary heterogeneity. Financing is a secondary empirical interaction and may enter the baseline only after independent evidence plus new authorization. | 2026-09-04 recentering instruction | ACTIVE / CONTROLLING |
