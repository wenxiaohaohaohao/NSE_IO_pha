...
\begin{longtable}{@{}P{0.17\linewidth}P{0.23\linewidth}P{0.15\linewidth}P{0.21\linewidth}P{0.16\linewidth}@{}}
\caption{Observable variables and their mapping to model objects} \\
\toprule
Variable & \shortstack[l]{Mathematical\\definition} & Economic meaning & Data source & \shortstack[l]{Regression/\\calibration role} \\
\midrule
\endfirsthead
...
External commercialization adoption &
\(\begin{aligned}[t]
\chi^{ext}_{ipt}
&=1\{\text{holder}_{ipt}\\
...
\end{aligned}\)
&
Whether the product is commercialized through separated holder--manufacturer roles &
NMPA product approvals, manufacturing licenses, provincial entrusted-manufacturing announcements &
Directly observed route adoption; core mechanism variable \\[1.2em]

Type-B external commercialization &
\(\begin{aligned}[t]
\chi^{B\to ext}_{ipt}
&=1\{\text{B-type holder uses}\\
&\quad \text{ext. manufacturer}\}
\end{aligned}\)
&
Most direct product-level realization of the MAH mechanism &
Product-level holder--manufacturer matching plus firm classification &
Sharpest empirical proxy for the external route \\[1.2em]

New original-drug products commercialized &
\(\begin{aligned}[t]
N_t
&=\#\{\text{new original-drug}\\
&\quad \text{products commercialized}\\
&\quad \text{at } t\}
\end{aligned}\)
&
Main policy outcome &
CDE/NMPA application, approval, and launch data &
Primary outcome variable \\[1.2em]
...
&\quad \text{entrants}\}
\end{aligned}\)
\newline or entrant share &
Upstream entry margin induced by lower external commercialization friction &
Business registry, applicant/holder identity, firm classification &
Auxiliary channel variable \\[1.2em]

Type-B conversion rate &
\(\mathrm{Conv}^B_t = \#R_t^B / \#P_t^B\)
&
Closest observable proxy for commercialization success &
CDE IND/project data + NMPA approvals + originator classification &
Core proxy for identifying the effect of \(\tau_{ext}\) \\[1.8em]

Pre-policy qualified manufacturing capacity &
\(\begin{aligned}[t]
Cap_{r,d,0}
&=\#\{\text{qualified mfrs.}\\
&\quad \text{in }(r,d), \text{ pre-MAH}\}
\end{aligned}\)
&
Exogenous supply-side shifter of external commercialization feasibility and governance friction &
Manufacturing-license database, dosage/process qualification scope &
Key heterogeneity variable; identifies where MAH should matter more \\[1.8em]

Producer-side outcomes &
Revenue growth, gross margin, operating margin, entrusted-manufacturing revenue &
How supply-side producers respond to higher external commercialization demand &
Annual reports, prospectuses, firm databases &
Supplementary equilibrium moments \\[1.2em]

Text proxies for commercialization &
Keyword frequency: ``entrusted manufacturing,'' ``MAH/holder,'' ``marketing release,'' ``CMO/CDMO'' &
Supplementary evidence on organizational route adoption &
Annual reports, prospectuses, announcements &
Auxiliary evidence only; not a core variable \\[1.2em]

Generic cash-flow block &
Generic sales, output, approvals &
Controls for the role of generic production in firm cash flow and industry equilibrium &
Annual reports and NMPA/CDE data &
Control and calibration block in the two-medicine environment \\[1.2em]

Review-speed reform controls &
Exposure to approval-delay reduction or drug-category-specific speed-up &
Separates MAH from contemporaneous review reform &
CDE/NMPA, drug-category data &
Control block in reduced-form analysis \\
\bottomrule
\end{longtable}
}
\end{landscape}
