...
\item \textbf{Research effort:} type-A chooses \((x_{A,t}^G,x_{A,t}^O)\); type-B chooses \(x_{B,t}^O\) only. Here \(x\) denotes innovation intensity, not an integer project count.
\item \textbf{Idea arrival:} innovation intensity generates generic or original-drug project arrivals probabilistically.
...
%------------------------------------------------
\begin{frame}{8. Type-B Innovation Intensity and Route Assignment}
\small
Type-B firms are the central mechanism objects in the paper. In the baseline they are
research-specialized entrants or organizations whose main economically relevant margin is original-drug innovation, so they do \textbf{not} choose generic-project effort.
...
\begin{itemize}
\item \(x_B^O\): type-B original-drug innovation intensity, interpretable as idea-arrival intensity or effort devoted to generating original-drug projects; it is not an integer count of ideas.
\item \(\Psi_B^O(z;p_m,M)\): expected value of one type-B original-drug idea after optimal route assignment.
\item Thus \(x_B^O\Psi_B^O\) is expected flow value: innovation intensity times per-idea expected value.
\item For each original-drug idea, the feasible route set is \(H=\{ext,int,tr,ab\}\).
\end{itemize}

\textbf{Key margin:} MAH raises the value of the external route, which raises \(\Psi_B^O\), and therefore lifts original-drug innovation intensity, continuation value, entry value, and realization.
\end{frame}
...
\item \textbf{Innovation-intensity margin:} higher per-idea value \(\Psi_B^O\) raises \(x_B^O\).
...
\(x_B^O\uparrow\) & Original-drug project starts, applications, R\&D effort, or idea-arrival proxies & Research-specialized firms raise original-drug innovation intensity \\
...
%------------------------------------------------
\begin{frame}{17. Data, Exposure Construction, and Type Outcomes}
\footnotesize
\begin{block}{Pre-reform exposure proxies}
Use \textbf{pre-reform} characteristics to measure exposure to the implementation bottleneck, not to claim that MAH-style type-B firms already existed before the reform.
\begin{itemize}
\item Integrated-incumbent exposure: original-drug activity plus relevant internal manufacturing capability.
\item Potential type-B exposure: research-oriented firms or entities with weak or no internal manufacturing capability.
\item Type-C exposure: qualified manufacturing specialists that can support external implementation.
\end{itemize}
\end{block}
...
\begin{itemize}
\item Baseline exposure: pre-reform research orientation combined with weak internal manufacturing capability, rather than post-reform MAH-holder status.
...
\begin{frame}{21. Supplementary Takeaway}
\small
This supplementary note clarifies how to interpret the empirical exposure construction on slide 17.

\begin{block}{Main point}
Use \textbf{pre-reform} firm or region characteristics to measure exposure to the implementation bottleneck created by the old research--production bundle.
\end{block}

\begin{itemize}
\item Do \textbf{not} define treatment as post-reform MAH-holder status.
\item Do \textbf{not} claim that MAH-style type-B firms already existed before the reform.
\item Do interpret high exposure as: more likely to benefit from lower external-implementation friction after MAH.
\end{itemize}

\begin{block}{Core exposure object}
\[
...
%------------------------------------------------
\begin{frame}{22. Exposure Construction}
\footnotesize
\begin{center}
\begin{tabular}{p{0.25\textwidth}p{0.23\textwidth}p{0.42\textwidth}}
\toprule
\textbf{Exposure type} & \textbf{Suggested proxy} & \textbf{Interpretation} \\
\midrule
Integrated-incumbent exposure & original-drug activity $\times$ manufacturing capability & Potential type-A firms that already combine research and production \\
Potential type-B exposure & research orientation $\times$ weak internal manufacturing & Core treated margin: firms constrained by the old bundle \\
Type-C exposure & qualified manufacturing capacity $\times$ weak research orientation & External implementation supply side \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.15cm}
\begin{block}{Concrete proxy ideas}
Research orientation can use original-drug applications, IND activity, patents, or R\&D intensity. Manufacturing capability can use production licenses, GMP status, dosage scope, and prior production records.
\end{block}

\begin{block}{A simple cross-classification}
High research + high manufacturing $\rightarrow$ integrated incumbent.\newline
High research + low manufacturing $\rightarrow$ potential B exposure.\newline
Low research + high manufacturing $\rightarrow$ potential C exposure.
\end{block}
\end{frame}

%------------------------------------------------
\begin{frame}{23. Empirical Design and Interpretation}
\footnotesize
...
\item $Post_t\times Exposure_i^{B,pre}$: firms more exposed to the old implementation bottleneck should react more after MAH.
\item $X_i^{pre}\times\gamma_t$: absorb differential pre-trends.
\end{itemize}

\begin{block}{Triple-difference version}
\[
\begin{aligned}
...
The coefficient $\beta$ captures whether reform has a stronger effect on firms that were both research-oriented and weak in internal manufacturing before MAH.
\end{block}
\end{frame}

%------------------------------------------------
\begin{frame}{24. Cautions}
\small
\begin{itemize}
\item Exposure should be measured before the reform; using post-reform holder status is endogenous.
\item MAH is an implementation-route policy, not a generic innovation shock.
\item The cleanest empirical story is first-layer objects: holder--producer separation, external-route use, realized original-drug products, and original-drug shares.
\item Type-C outcomes are supporting evidence for the contract-manufacturing market, not the final target.
\end{itemize}

\begin{block}{Bottom line}
This supplement makes the slide deck's mechanism claim sharper: MAH matters because it lowers the friction of external commercialization for pre-reform exposed firms.
\end{block}
\end{frame}
