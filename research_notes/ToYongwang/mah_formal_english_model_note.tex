...
\title{A Formal Model Note for the MAH Paper:\\
States, Controls, Static Inputs, Prices, and Payoff Objects}
\author{}
\date{}
...
\caption{Classification of state variables, controls, prices, and payoff objects}
\label{tab:objects}
\begin{tabular}{@{}P{0.22\textwidth}P{0.20\textwidth}P{0.54\textwidth}@{}}
\toprule
Object & Type & Interpretation \\
\midrule
\multicolumn{3}{@{}l}{\textbf{Panel A. States and prices}} \\
\addlinespace[0.2em]

$z$ & State variable & Persistent firm capability state. \\[0.45em]

$s\in\{A,B,C\}$ & Discrete state variable & Current organizational form of the firm. \\[0.45em]

$w$ & Price & Wage or composite factor price. \\[0.45em]

$p_m$ & Price & Price of qualified contract-manufacturing services. \\[0.7em]

\multicolumn{3}{@{}l}{\textbf{Panel B. Static inputs and dynamic controls}} \\
\addlinespace[0.2em]

$n,k,m$ & Static input choices & Labor, capital services, and intermediate inputs used in current-period production. \\[0.45em]

$x_A,x_B$ & Dynamic control variables & Original-drug idea-generation intensities chosen by type-$A$ and type-$B$ firms. \\[0.45em]

$h$ & Dynamic control variable & Commercialization route chosen for a type-$B$ idea, e.g. externalize, transfer, abandon. \\[0.45em]

$j$ & Dynamic control variable & Next organizational choice (stay, switch, exit). \\[0.7em]

\multicolumn{3}{@{}l}{\textbf{Panel C. Frictions, payoff objects, and values}} \\
\addlinespace[0.2em]

$\kappa_{sj}$ & Adjustment-cost object & Organizational switching cost from current type $s$ to chosen type $j$. \\[0.45em]

$\tau_{ext}$ & Route-specific friction & Governance/compliance friction of external commercialization through qualified outside manufacturing. \\[0.45em]

$\bar{\pi}_A(z;w)$ & Optimized static payoff & Current-period operating profit of type $A$ after static production inputs are optimized out. \\[0.45em]

$\bar{\pi}_C(z;p_m,w)$ & Optimized static payoff & Current-period operating profit of type $C$ after static production inputs are optimized out. \\[0.45em]

$\Psi_h(z)$ & Per-idea net value object & Expected net value of one original-drug idea under route $h$. \\[0.45em]

$\begin{aligned}[t]
\Pi_A(z),\,\Pi_B(z),\\
\Pi_C(z)
\end{aligned}$ & Optimized flow payoff objects & Current-period payoffs that enter the Bellman equation. \\[0.45em]

$\begin{aligned}[t]
V_A(z),\,V_B(z),\\
V_C(z)
\end{aligned}$ & Value functions & Continuation values under each organizational type. \\
